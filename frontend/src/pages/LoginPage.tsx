import { useState, useEffect } from 'react';
import { useNavigate, useSearchParams, Link } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
import { useAuth } from '../contexts/AuthContext';
import { dashboardPathForUserType } from '../lib/authRedirect';
import { User, Lock, Sparkles, Eye, EyeOff } from 'lucide-react';
import { GymCodeIcon } from '../components/GymCodeIcon';
import { LanguagePicker } from '../components/LanguagePicker';

const STUDENT_LOCKOUT_MINUTES = 5;
const STUDENT_LOCKOUT_MS = STUDENT_LOCKOUT_MINUTES * 60 * 1000;
const STUDENT_BLOCKED_UNTIL_KEY = 'studentLoginBlockedUntil';
const MAX_STUDENT_ATTEMPTS = 2;

export default function LoginPage() {
  const { t } = useTranslation();
  const [searchParams] = useSearchParams();
  const [userType, setUserType] = useState<'personal' | 'student' | null>(null);
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);
  const [accessCode, setAccessCode] = useState('');
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const [failedStudentAttempts, setFailedStudentAttempts] = useState(0);
  const [studentBlockedUntil, setStudentBlockedUntil] = useState<number | null>(null);

  const { login, loginStudent, isAuthenticated, userType: authUserType, loading: authLoading } = useAuth();
  const navigate = useNavigate();

  const lockoutMessage = t('errors.tooManyAttempts');

  useEffect(() => {
    if (authLoading || !isAuthenticated || !authUserType) return;
    navigate(dashboardPathForUserType(authUserType), { replace: true });
  }, [authLoading, isAuthenticated, authUserType, navigate]);

  useEffect(() => {
    const code = searchParams.get('code');
    const tipo = searchParams.get('tipo');
    if (code && (tipo === 'aluno' || /^[A-Z0-9]{5}$/i.test(code))) {
      setUserType('student');
      setAccessCode(code.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 5));
    }
  }, [searchParams]);

  const isStudentBlocked =
    studentBlockedUntil !== null && Date.now() < studentBlockedUntil;

  useEffect(() => {
    if (userType !== 'student') return;
    const stored = sessionStorage.getItem(STUDENT_BLOCKED_UNTIL_KEY);
    if (!stored) return;
    const until = Number(stored);
    if (until > Date.now()) {
      setStudentBlockedUntil(until);
      setError(lockoutMessage);
    } else {
      sessionStorage.removeItem(STUDENT_BLOCKED_UNTIL_KEY);
      setStudentBlockedUntil(null);
    }
  }, [userType, lockoutMessage]);

  useEffect(() => {
    if (!studentBlockedUntil || studentBlockedUntil <= Date.now()) return;
    const timer = setInterval(() => {
      if (Date.now() >= studentBlockedUntil) {
        setStudentBlockedUntil(null);
        setError('');
        sessionStorage.removeItem(STUDENT_BLOCKED_UNTIL_KEY);
        clearInterval(timer);
      }
    }, 1000);
    return () => clearInterval(timer);
  }, [studentBlockedUntil]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    setLoading(true);

    try {
      if (userType === 'personal') {
        await login(email, password, 'personal');
        navigate('/personal/home', { replace: true });
      } else if (userType === 'student') {
        if (isStudentBlocked) {
          setError(lockoutMessage);
          setLoading(false);
          return;
        }
        await loginStudent(accessCode);
        setFailedStudentAttempts(0);
        setStudentBlockedUntil(null);
        sessionStorage.removeItem(STUDENT_BLOCKED_UNTIL_KEY);
        navigate('/student/dashboard', { replace: true });
      }
    } catch (err: unknown) {
      const message =
        (err as { message?: string })?.message || t('errors.invalidCode');
      setError(message);
      if (userType === 'student') {
        const nextAttempts = failedStudentAttempts + 1;
        setFailedStudentAttempts(nextAttempts);
        if (nextAttempts >= MAX_STUDENT_ATTEMPTS) {
          const until = Date.now() + STUDENT_LOCKOUT_MS;
          setStudentBlockedUntil(until);
          sessionStorage.setItem(STUDENT_BLOCKED_UNTIL_KEY, String(until));
          setError(lockoutMessage);
        }
      }
    } finally {
      setLoading(false);
    }
  };

  const loginCard = (content: React.ReactNode) => (
    <div className="glass-effect rounded-2xl md:rounded-3xl shadow-strong p-6 md:p-10 border border-white/10 relative">
      <div className="absolute top-4 right-4 z-30 md:top-6 md:right-6">
        <LanguagePicker />
      </div>
      {content}
    </div>
  );

  const loginShell = (children: React.ReactNode) => (
    <div className="min-h-screen w-full overflow-x-hidden bg-gradient-dark flex items-center justify-center p-3 md:p-4 relative">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-10 left-5 w-48 h-48 md:w-72 md:h-72 bg-primary-500/20 rounded-full blur-3xl" />
        <div className="absolute bottom-10 right-5 w-64 h-64 md:w-96 md:h-96 bg-accent-500/20 rounded-full blur-3xl" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 md:w-[600px] md:h-[600px] bg-primary-600/10 rounded-full blur-3xl" />
      </div>

      <div className="w-full max-w-md relative px-4">{children}</div>
    </div>
  );

  if (!userType) {
    return loginShell(
      loginCard(
        <>
        <div className="text-center mb-6 md:mb-10">
          <div className="inline-flex items-center justify-center w-14 h-14 md:w-20 md:h-20 bg-gradient-accent rounded-xl md:rounded-2xl mb-4 md:mb-6 shadow-medium">
            <GymCodeIcon size={40} className="text-white md:w-10 md:h-10" />
          </div>
          <h1 className="text-2xl md:text-4xl font-display font-bold text-white mb-2 md:mb-3">
            Gym<span className="bg-gradient-accent bg-clip-text text-transparent">Code</span>
          </h1>
          <p className="text-slate-300 text-sm md:text-lg">{t('login.tagline')}</p>
          <div className="flex items-center justify-center gap-2 mt-3 md:mt-4">
            <Sparkles className="w-3 h-3 md:w-4 md:h-4 text-accent-400" />
            <p className="text-slate-400 text-xs md:text-sm">{t('login.chooseProfile')}</p>
          </div>
        </div>

        <div className="space-y-3 md:space-y-4">
          <button
            onClick={() => setUserType('personal')}
            className="group w-full flex items-center justify-between px-4 py-3 md:px-8 md:py-5 bg-gradient-accent text-white rounded-xl hover:shadow-strong transform hover:-translate-y-1 transition-all duration-300 font-display font-semibold text-base md:text-lg relative overflow-hidden"
          >
            <div className="flex items-center gap-3 md:gap-4 relative z-10">
              <div className="w-10 h-10 md:w-12 md:h-12 bg-white/20 rounded-lg md:rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                <User className="w-5 h-5 md:w-6 md:h-6" />
              </div>
              <div className="text-left">
                <div className="text-sm md:text-base">{t('login.personalTrainer')}</div>
                <div className="text-xs font-normal opacity-90 hidden sm:block">
                  {t('login.personalSubtitle')}
                </div>
              </div>
            </div>
            <div className="text-xl md:text-2xl group-hover:translate-x-1 transition-transform">→</div>
            <div className="absolute inset-0 bg-gradient-to-r from-accent-600/0 via-accent-400/20 to-accent-600/0 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700" />
          </button>

          <button
            onClick={() => setUserType('student')}
            className="group w-full flex items-center justify-between px-4 py-3 md:px-8 md:py-5 bg-dark-700/50 backdrop-blur-sm text-white rounded-xl hover:bg-dark-700 hover:shadow-medium transform hover:-translate-y-1 transition-all duration-300 font-display font-semibold text-base md:text-lg border border-dark-600"
          >
            <div className="flex items-center gap-3 md:gap-4">
              <div className="w-10 h-10 md:w-12 md:h-12 bg-white/10 rounded-lg md:rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                <GymCodeIcon size={24} className="text-white" />
              </div>
              <div className="text-left">
                <div className="text-sm md:text-base">{t('login.student')}</div>
                <div className="text-xs font-normal opacity-75 hidden sm:block">
                  {t('login.studentSubtitle')}
                </div>
              </div>
            </div>
            <div className="text-xl md:text-2xl group-hover:translate-x-1 transition-transform">→</div>
          </button>
        </div>
        </>
      )
    );
  }

  const isWaitError = error.includes(t('errors.waitBlocked')) || isStudentBlocked;

  return loginShell(
    loginCard(
      <>
      <div className="text-center mb-6 md:mb-8">
        <div className="inline-flex items-center justify-center w-12 h-12 md:w-16 md:h-16 bg-gradient-accent rounded-xl md:rounded-2xl mb-4 md:mb-5 shadow-medium">
          <GymCodeIcon size={32} className="text-white" />
        </div>
        <h1 className="text-xl md:text-3xl font-display font-bold text-white mb-2">
          {userType === 'personal' ? t('login.personalArea') : t('login.studentArea')}
        </h1>
        {!(userType === 'student' && error) && (
          <button
            type="button"
            onClick={() => setUserType(null)}
            className="text-xs md:text-sm text-accent-400 hover:text-accent-300 font-medium transition-colors inline-flex items-center gap-1"
          >
            ← {t('login.backToGymCode')}
          </button>
        )}
      </div>

      <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
        {error && !(userType === 'student' && (isWaitError || isStudentBlocked)) && (
          <div
            className={`px-3 py-2 md:px-4 md:py-3 rounded-xl backdrop-blur-sm text-xs md:text-sm ${
              isWaitError
                ? 'bg-amber-500/10 border border-amber-500/30 text-amber-400'
                : 'bg-red-500/10 border border-red-500/30 text-red-400'
            }`}
          >
            {error}
          </div>
        )}

        {userType === 'personal' ? (
          <>
            <div>
              <label className="block text-xs md:text-sm font-semibold text-dark-200 mb-2">
                {t('login.email')}
              </label>
              <div className="relative">
                <User className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 w-4 h-4 md:w-5 md:h-5 text-dark-400" />
                <input
                  type="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="w-full pl-10 md:pl-12 pr-3 md:pr-4 py-2.5 md:py-3.5 bg-dark-800/50 border-2 border-dark-700 rounded-xl focus:border-accent-500 focus:ring-4 focus:ring-accent-500/20 outline-none transition-all text-white placeholder:text-slate-500 text-sm md:text-base"
                  placeholder={t('login.emailPlaceholder')}
                  required
                />
              </div>
            </div>

            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-xs md:text-sm font-semibold text-dark-200">
                  {t('login.password')}
                </label>
                <Link
                  to="/forgot-password"
                  className="text-xs text-accent-400 hover:text-accent-300 font-medium transition-colors"
                >
                  {t('login.forgotPassword')}
                </Link>
              </div>
              <div className="relative">
                <Lock className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 w-4 h-4 md:w-5 md:h-5 text-dark-400" />
                <input
                  type={showPassword ? 'text' : 'password'}
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  className="w-full pl-10 md:pl-12 pr-10 md:pr-12 py-2.5 md:py-3.5 bg-dark-800/50 border-2 border-dark-700 rounded-xl focus:border-accent-500 focus:ring-4 focus:ring-accent-500/20 outline-none transition-all text-white placeholder:text-slate-500 text-sm md:text-base"
                  placeholder="••••••••"
                  required
                />
                <button
                  type="button"
                  onClick={() => setShowPassword((v) => !v)}
                  className="absolute right-3 md:right-4 top-1/2 -translate-y-1/2 text-dark-400 hover:text-slate-300 transition-colors"
                  aria-label={showPassword ? t('login.hidePassword') : t('login.showPassword')}
                >
                  {showPassword ? (
                    <EyeOff className="w-4 h-4 md:w-5 md:h-5" />
                  ) : (
                    <Eye className="w-4 h-4 md:w-5 md:h-5" />
                  )}
                </button>
              </div>
            </div>
          </>
        ) : isWaitError ? (
          <div className="rounded-xl border-2 border-amber-500/40 bg-amber-500/10 p-4 md:p-5 text-center">
            <p className="text-sm font-semibold text-amber-400 mb-1">{t('login.blockedTitle')}</p>
            <p className="text-xs text-amber-200/90">{error || lockoutMessage}</p>
            <p className="text-xs text-slate-500 mt-2">{t('login.tryAgainIn5Min')}</p>
          </div>
        ) : (
          <div>
            <label className="block text-xs md:text-sm font-semibold text-dark-200 mb-2 text-center">
              {t('login.accessCode')}
            </label>
            <input
              type="text"
              value={accessCode}
              onChange={(e) =>
                setAccessCode(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 5))
              }
              className="w-full px-3 md:px-4 py-3 md:py-4 bg-dark-800/50 border-2 border-dark-700 rounded-xl focus:border-accent-500 focus:ring-4 focus:ring-accent-500/20 outline-none text-center text-2xl md:text-3xl font-bold tracking-[0.2em] md:tracking-[0.4em] text-white placeholder:text-slate-600 placeholder:tracking-[0.2em] md:placeholder:tracking-[0.4em]"
              placeholder="A1234"
              maxLength={5}
              required
            />
            <p className="text-xs text-slate-400 text-center mt-2">{t('login.accessCodeHint')}</p>
            {error && (
              <p className="text-xs text-slate-500 text-center mt-1">{t('login.accessCodeAttempts')}</p>
            )}
          </div>
        )}

        <button
          type="submit"
          disabled={loading || (userType === 'student' && (isWaitError || isStudentBlocked))}
          className="btn-primary w-full disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
        >
          {loading ? (
            <span className="flex items-center justify-center gap-2">
              <div className="w-4 h-4 md:w-5 md:h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
              {t('login.signingIn')}
            </span>
          ) : (
            t('login.signIn')
          )}
        </button>

        {userType === 'personal' && (
          <div className="text-center pt-2">
            <p className="text-xs md:text-sm text-slate-300">
              {t('login.noAccount')}{' '}
              <Link
                to="/register"
                className="text-accent-400 hover:text-accent-300 font-semibold transition-colors"
              >
                {t('login.registerHere')}
              </Link>
            </p>
          </div>
        )}
      </form>
      </>
    )
  );
}
